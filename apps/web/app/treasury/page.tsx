            ...
            <div className="flex-1 relative bg-[#080808]" ref={reactFlowWrapper}>
                ...
                >
                    ...
                    <Panel position="top-left" className={`
                        ...
                        ${sidebarOpen ? 'ml-4' : 'ml-16 lg:ml-20'}
                    `}>
                        <div className="bg-[#121212]/90 backdrop-blur-xl border border-white/10 p-3 sm:p-5 rounded-2xl sm:rounded-3xl shadow-2xl flex items-center gap-3 sm:gap-4 max-w-[200px] sm:max-w-none overflow-hidden">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                    ...
                                    <div className="ml-2 pl-2 border-l border-white/10 flex items-center gap-1.5">
                                        ...
                                        <span className="text-[7px] font-mono text-gray-700 uppercase tracking-widest">Ethics Aligned</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <Panel position="top-right" className="pt-4 flex flex-col sm:flex-row gap-2 sm:gap-3 items-end sm:items-center">
                        <button
                            onClick={() => handleSave(false)}
                            disabled={isSaving}
                            className="bg-[#121212]/90 backdrop-blur-xl border border-white/10 px-4 sm:px-6 py-2.5 sm:py-3.5 rounded-xl sm:rounded-2xl font-mono text-[9px] sm:text-[10px] font-black tracking-[0.15em] sm:tracking-[0.25em] text-gray-500 hover:text-white hover:border-white/20 transition-all flex items-center gap-2 sm:gap-3 shadow-2xl active:scale-95"
                        >
                            <Save size={14} className="sm:w-4 sm:h-4" /> {t.treasury.controls.save_draft}
                        </button>
                        <div className="flex gap-2 w-full sm:w-auto">
                            <button
                                onClick={() => handleSave(true)}
                                disabled={isSaving}
                                className="flex-1 sm:flex-none bg-stellar-teal px-4 sm:px-8 py-2.5 sm:py-3.5 rounded-xl sm:rounded-2xl font-mono text-[9px] sm:text-[10px] font-black tracking-[0.15em] sm:tracking-[0.25em] text-black hover:scale-105 active:scale-95 shadow-[0_0_30px_rgba(45,235,232,0.4)] transition-all flex items-center justify-center gap-2 sm:gap-3"
                            >
                                <Play size={14} fill="currentColor" className="sm:w-4 sm:h-4" /> {t.treasury.controls.run}
                            </button>
                            <button
                                onClick={handlePublishOnChain}
                                className="flex-1 sm:flex-none bg-purple-600/20 text-purple-400 border border-purple-600/50 px-4 sm:px-8 py-2.5 sm:py-3.5 rounded-xl sm:rounded-2xl font-mono text-[9px] sm:text-[10px] font-black tracking-[0.15em] sm:tracking-[0.25em] hover:bg-purple-600 hover:text-white shadow-[0_0_30px_rgba(168,85,247,0.2)] hover:shadow-[0_0_40px_rgba(168,85,247,0.6)] transition-all flex items-center justify-center gap-2 sm:gap-3 active:scale-95"
                            >
                                <Database size={14} className="sm:w-4 sm:h-4" /> {t.treasury.controls.publish}
                            </button>
                        </div>
                    </Panel>

                    <Panel position="bottom-right" className="flex flex-col gap-3">
                        <button onClick={() => { fetchHistory(); setShowHistory(true); }} className="w-14 h-14 bg-[#121212]/90 backdrop-blur-xl border border-white/10 rounded-2xl flex items-center justify-center text-gray-500 hover:text-stellar-teal hover:border-stellar-teal/30 shadow-2xl transition-all hover:scale-110 active:scale-90 group relative">
                            <History size={20} />
                            <span className="absolute right-full mr-4 bg-black text-white text-[9px] font-black px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity uppercase tracking-widest">{t.treasury.controls.history_tooltip}</span>
                        </button>
                        <button onClick={handleExport} className="w-14 h-14 bg-[#121212]/90 backdrop-blur-xl border border-white/10 rounded-2xl flex items-center justify-center text-gray-500 hover:text-white shadow-2xl transition-all hover:scale-110 active:scale-90 group relative">
                            <Download size={20} />
                            <span className="absolute right-full mr-4 bg-black text-white text-[9px] font-black px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity uppercase tracking-widest">{t.treasury.controls.export_tooltip}</span>
                        </button>
                    </Panel>
                </ReactFlow>
                        ...
                        >
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
                                {isLoadingHistory ? (
                                    ...
                                    >
                                        <div className="absolute top-0 right-0 px-3 py-1 bg-stellar-teal/10 text-stellar-teal text-[8px] font-black uppercase tracking-tighter rounded-bl-xl opacity-0 group-hover:opacity-100 transition-opacity">Deploy Hub</div>
                                        ...
                                        <div className="flex-1 text-left min-w-0">
                                            ...
                                            <div className="flex items-center gap-3 text-[10px] font-black font-mono text-gray-600 uppercase tracking-tighter">
                                                ...
                                                <span className="text-stellar-teal font-black">{strat.yield} APY</span>
                                            </div>
                                        </div>
