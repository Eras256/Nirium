...
server.tool(
    'get_system_health',
    'System health: Horizon, Soroban RPC, WebSocket, IPFS, LLM provider status. Requires NIRIUM_API_KEY env var.',
    {},
    async () => {
        const res = await fetch(`${API_URL}/api/system/health`, { headers: authHeaders() });
        const data = await res.json();
        return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
    },
);
        ...
        return {
            content: [{
                ...
                text: JSON.stringify({
                    ...
                    authenticatedTools: ['start_loop', 'stop_loop', 'get_system_health'],
