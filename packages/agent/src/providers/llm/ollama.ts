...
export class OllamaProvider extends LLMProvider {
     ...
     */
    private generateLocalDecision(market: MarketState): AIDecision {
        // Check for path arbitrage opportunity
        if (market.pathPaymentRoutes.length > 0) {
            const bestRoute = market.pathPaymentRoutes.reduce((best, route) =>
                route.profitPercentage > best.profitPercentage ? route : best
                , market.pathPaymentRoutes[0]);
