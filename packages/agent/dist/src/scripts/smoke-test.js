...
async function main() {
    ...
    await runTest('Soroban RPC Health', async () => {
        ...
        if (data?.result?.status !== 'healthy')
            throw new Error(`Status: ${data?.result?.status}`);
        return 'Healthy';
    });
    // 3. XLM Price Fetch
    ...
    await runTest('Agent Market Data', async () => {
        if (!smokeToken)
            throw new Error('No demo token available — skipping');
        ...
        if (!res.ok)
            throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
        if (!data.xlmPrice || data.xlmPrice <= 0)
            throw new Error(`Invalid XLM price: ${data.xlmPrice}`);
