...
export async function deleteWebhook(id) {
    ...
    webhooks.delete(id);
    if (existed)
        console.log(`[Webhook] Deleted: ${id} (memory)`);
    return existed;
}
/**
...
export async function dispatchWebhookEvent(event, payload) {
    ...
    if (SUPABASE_AVAILABLE) {
        try {
            ...
            if (data) {
                matching = data.map((row) => ({ id: row.id, userId: row.user_id, url: row.url, events: row.events, secret: row.secret, active: row.active, failureCount: row.failure_count, createdAt: row.created_at }));
            }
        }
        catch { /* fallback */ }
    }
