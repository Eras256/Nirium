...
import { authMiddleware, adminMiddleware, generateToken, generateApiKey, getUserApiKeys, revokeApiKey, } from './middleware/index.js';
...
app.post('/api/auth/keys', authMiddleware, adminMiddleware, (req, res) => {
    const authReq = req;
    const { name } = req.body;
    const apiKey = generateApiKey(authReq.user.userId, name || 'Default Key', ['user']);
...
app.get('/api/auth/keys', authMiddleware, (req, res) => {
    const authReq = req;
    const keys = getUserApiKeys(authReq.user.userId);
    res.json({ keys });
});
app.delete('/api/auth/keys/:id', authMiddleware, (req, res) => {
...
// EXECUTION ENDPOINTS
// ═══════════════════════════════════════════════════════════════
app.post('/api/execute', authMiddleware, aggressiveLimiter, async (req, res) => {
