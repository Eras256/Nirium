...
// Contratos a indexar
const CONTRACTS = [
    'CDHDX63NUYSFCIPJTTS46N5PYLTI7J5WIAIOP7TZSPBNUTLI32AY7GA2', // NiriumVault activo
];
...
const supabase = (SUPABASE_URL && SUPABASE_KEY)
    ...
    : null;
async function logToSupabase(agentId, message, level = 'info') {
    if (!supabase)
        return;
...
function extractTopics(event) {
    ...
    try {
        const topics = (event.topicDecoded ?? event.topic);
        return topics.map(t => {
            if (typeof t === 'string')
                return t;
...
const txSenderCache = new Map();
/** Determina el wallet que disparó el evento buscando en Horizon */
async function getTxSender(txHash) {
    if (txSenderCache.has(txHash))
        return txSenderCache.get(txHash);
    const controller = new AbortController();
...
// ─── ELO engine ──────────────────────
function updateElo(walletAddress, won) {
...
function getOrInitStats(walletAddress, agentName) {
...
async function processEvent(event) {
    ...
    if (cat === 'pool' && action === 'created') {
        ...
        console.log(chalk.cyan(`📊 [${event.ledger}] pool/created │ ${sender.slice(0, 8)}… │ ${STELLAR_EXPERT}/tx/${event.txHash}`));
    }
    else if (cat === 'vault' && action === 'created') {
        stats.vaults_created++;
        updateElo(sender, true);
        console.log(chalk.green(`🏦 [${event.ledger}] vault/created │ ${sender.slice(0, 8)}…`));
        ...
        updateElo(sender, true);
        console.log(chalk.blue(`🤖 [${event.ledger}] agent/delegated │ ${sender.slice(0, 8)}…`));
    }
    else if (cat === 'flash' && (action === 'exec' || action === 'done')) {
        stats.flash_loans++;
        updateElo(sender, true);
        console.log(chalk.yellow(`⚡ [${event.ledger}] flash/loan │ ${sender.slice(0, 8)}… │ ${STELLAR_EXPERT}/tx/${event.txHash}`));
...
async function flushToSupabase() {
    ...
    try {
        ...
        if (error)
            console.warn(chalk.yellow(`⚠️  Supabase flush: ${error.message}`));
    }
    catch (e) {
        console.error(chalk.red('Log Error:'), e);
    }
...
const AGENT_SECRETS_CACHE = {};
function findAgentName(walletAddress) {
    const NAMES = [
...
async function runIndexer() {
    ...
    while (true) {
        ...
        try {
            ...
            console.warn(chalk.yellow(`⚠️  Indexer tick error: ${err.message}`));
        }
        await new Promise(r => setTimeout(r, POLL_INTERVAL_MS));
    }
}
runIndexer().catch(console.error);
//# sourceMappingURL=nirium_indexer.js.map